  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm">
      ...
      <ul className="divide-y divide-gray-200">
        {documents?.map(
          ({ id, document_name, file_type, publication_date }) => (
            <li key={id} className="py-4 flex items-center justify-between">
              <div className="flex items-center">
                <FileTextOutlined
                  style={{ fontSize: "24px", color: "var(--color-blue-600)" }}
                  className="me-6"
                />
                <div>
                  <p className="font-medium text-gray-900">
                    {document_name
                      ? `${document_name}${!document_name.includes(".") ? "." + mimeToExtension[file_type] : ""}`
                      : "Unnamed Document"}
                  </p>
                  <p className="text-sm text-gray-500">{`Notarized on: ${publication_date}`}</p>
                </div>
              </div>
              <NavLink to={`/document/${id}/view`}>
