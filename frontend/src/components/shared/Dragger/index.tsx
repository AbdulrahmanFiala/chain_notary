  ...
  const props: UploadProps = {
    ...
    onChange: async (info) => {
      ...
      if (status === 'done') {
        ...
        form.setFieldsValue({ file_data, file_type, file_size: BigInt(size || 0), file_hash, name: name || info.file.name });
        ...
        if (info.file.name?.toLowerCase().endsWith('.xbrl')) {
          try {
            ...
            if (xbrlData.companyName) formData.company_name = xbrlData.companyName;
            ...
            if (xbrlData.totalAssets) balanceSheetData.total_assets = xbrlData.totalAssets;
            if (xbrlData.totalLiabilities) balanceSheetData.total_liabilities = xbrlData.totalLiabilities;
            if (xbrlData.totalEquity) balanceSheetData.total_equity = xbrlData.totalEquity;
            if (xbrlData.totalLiabilitiesAndEquity) balanceSheetData.total_liabilities_and_equity = xbrlData.totalLiabilitiesAndEquity;
            
            if (Object.keys(balanceSheetData).length > 0) {
              earningReleaseData.consolidated_balance_sheet_data = balanceSheetData;
            }
            
            // Income data
            const incomeData: Record<string, number> = {};
            if (xbrlData.grossProfit) incomeData.gross_profit = xbrlData.grossProfit;
            if (xbrlData.netProfit) incomeData.net_profit = xbrlData.netProfit;
            if (xbrlData.operatingProfit) incomeData.operating_profit = xbrlData.operatingProfit;
            if (xbrlData.profitBeforeTax) incomeData.profit_before_tax = xbrlData.profitBeforeTax;
            ...
            message.success(`${info.file.name} uploaded and form auto-filled successfully.`);
          } catch (error) {
